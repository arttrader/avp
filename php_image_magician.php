<?php
   ...
   #  0-07-12  J.C.O   1.0 
   #  02-12-15  JH  Exception handler ImageMagicianException added
   #
...
## --------------------------------------------------------
  ...
  # Date:     27-02-08    
    ...
    if (!$this->testGDInstalled()) { 
    	ImageMagicianException('The GD Library is not installed.'); 
    }
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/ 
  ...
  #       forceStretch can be applied to options 1,2,3 and 4  
    ...
    if (!$this->image) { 
    	throw new ImageMagicianException('file ' . $this->getFileName() .' is missing or invalid');
    }
...
## --------------------------------------------------------
    ...
    # Credit:   Incorporates Joe Lencioni (August 6, 2008) code 
  {
    ...
    {
      throw new ImageMagicianException('Sharpening required PHP 5.1.0 or greater.');     
    }
  }
    ...
    # 
  {
    if (is_array($option)) {
      if (strtolower($option[0]) == 'crop' && count($option) == 2) {
        ...
        throw new ImageMagicianException('Crop resize option array is badly formatted.');
      }
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/   
  ...
  #     
  {
    ...
    if (count($this->captionBoxPositionArray) == 4) {
      ...
      if ($this->debug) { 
      	throw new ImageMagicianException('No caption box found.'); 
      } else{ return false; }
    }
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/   
  ...
  { 
    ...
    if (!$this->testEXIFInstalled()) { if ($this->debug) { throw new ImageMagicianException('The EXIF Library is not installed.'); }else{ return array(); }};
    if (!file_exists($this->fileName)) { if ($this->debug) { throw new ImageMagicianException('Image not found.'); }else{ return array(); }};
    if ($this->fileExtension != '.jpg') { if ($this->debug) { throw new ImageMagicianException('Metadata not supported for this image type.'); }else{ return array(); }};
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/   
    ...
    #
  {
    ...
    if (!$fontColor) {
    	echo "font color allocation failed!\n";
	}
  ...
  private function getTextFont($font)
  {
    ...
    if ($font == null || !file_exists($font)) {
      ...
      if (!file_exists($font)) {
        ...
        // *** If not, return false
        if ($this->debug) { throw new ImageMagicianException('Font not found'); }else{ return false; }           
      }
    }   
...
## -------------------------------------------------------- 
    ...
    if (!file_exists($file) && !$this->checkStringStartsWith('http://', $file)) { if ($this->debug) { throw new ImageMagicianException('Image not found.'); }else{ throw new ImageMagicianException(); }};
...
## -------------------------------------------------------- 
  ...
  #       third party class to save as bmp.
    ...
    if (!is_resource($this->imageResized)) { if ($this->debug) { throw new ImageMagicianException('saveImage: This is not a resource.'); }else{ throw new ImageMagicianException(); }}      
    $fileInfoArray = pathInfo($savePath);
    clearstatcache();
    if (!is_writable($fileInfoArray['dirname'])) {  if ($this->debug) { throw new ImageMagicianException('The path is not writable. Please check your permissions.'); }else{ throw new ImageMagicianException(); }} 
...
/*	Exception Handling 
	2015-12-2
*/
class ImageMagicianException extends Exception {
/*	public $desc;
