...
if ($_POST) {
	...
	if ($send==="変更") {
		$mi = $_POST['mi'];
		$sql = sprintf("update category set name='%s',description='%s' where category_id=%u", $title,$desc,$mi);
		$result = getDB($sql,false);
		$message = "カテゴリデータを変更しました";		
	} else if ($send==="追加") {
		if ($title) {
			$sql = sprintf("insert into category(name,description,group_id) values('%s','%s',%u)", $title,$desc,$userGroup);
			$result = getDB($sql,false);
		} else
			$message = "カテゴリが入力されてません";
	} else if ($send==="削除") {
		if (isset($_POST['delete'])) {
			if (is_array($_POST['delete'])) {
				foreach($_POST['delete'] as $i) {
					$catId = $_POST['catId'.$i];
					if ($debugmode) echo "deleting ".$catId."<br /><br />\n";
					$sql = sprintf("delete from category where category_id=%u;", $catId);
					$result = getDB($sql,false);
				}
			} else {
				$i = $_POST['delete'];
	...
	if (isset($_GET['mi'])) {
		$mi = $_GET['mi'];
		// load
		$sql = "select * from category where category_id=".$mi;
		$result = getDB($sql);
		if (count($result)) {
			$rec = $result[0];
			$title = $rec['name'];
...
function getNumList() {
	global $userGroup;
	$sql = "select count(*) n from category where group_id=".$userGroup;
	$result = getDB($sql);
	return $result[0]['n'];
}
...
function getCatList($offset, $numItems) {
	global $userGroup;
    $sql = "select * from category where group_id=$userGroup order by category_id desc limit ".$offset.",".$numItems;
	$result = getDB($sql);
	return $result;
}
?>
...
<div class="subtitle">カスタムカテゴリ管理</div>
